  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-1.5">
        <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white/70 backdrop-blur-sm ring-1 ring-slate-900/20">
          <span className="text-xl font-black tracking-tight text-slate-900 drop-shadow">
            {makeWord.slice(0, 1)}{modelWord.slice(0, 1) || makeWord.slice(1, 2)}
          </span>
        </div>
        <div className="text-center">
          <p className="text-[11px] font-bold uppercase tracking-widest text-slate-600">{makeWord}</p>
          {modelWord && <p className="text-[10px] text-slate-500">{modelWord}</p>}
        </div>
      </div>
      {/* year + fuel pills */}
      <div className="flex items-center gap-1.5">
        <span className="rounded-full bg-white/65 px-2 py-0.5 text-[10px] font-medium text-slate-600">
          {year}
        </span>
        {isElectric && (
          <span className="rounded-full bg-emerald-500/30 px-2 py-0.5 text-[10px] font-medium text-emerald-700">
            ⚡
          </span>
        )}
      </div>
...
function TrustRing({ score }: { score: number }) {
  const bg =
    ...
    score >= 60 ? "bg-amber-500/100" : "bg-red-500";

  return (
    <div
      className={`absolute top-3 right-3 flex h-9 w-9 flex-col items-center justify-center rounded-full ${bg} shadow-lg ring-2 ring-slate-900/40`}
      title={`Etibar xalı: ${score}/100`}
    >
      <span className="text-[11px] font-bold leading-none text-slate-900">{score}</span>
    </div>
  );
}
...
function PlanRibbon({ planType }: { planType?: "free" | "standard" | "vip" }) {
  ...
  if (planType === "vip") {
    return (
      <div className="absolute left-0 top-4 flex items-center gap-1 rounded-r-full bg-gradient-to-r from-amber-500 to-yellow-400 pl-3 pr-3 py-1 text-[11px] font-bold text-amber-950 shadow">
        ★ VIP
      </div>
    );
  }
        ...
        isPremium
          ...
          : `rounded-2xl bg-white border hover:shadow-xl ${
              isPromoted
                ...
                : "border-slate-900/10 shadow-sm hover:border-slate-900/20"
            }`
      }`}
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <div>
          <h3 className={`line-clamp-1 font-semibold transition-colors ${
            isPremium
              ? "uppercase tracking-wide text-slate-900 group-hover:text-[#0057FF]"
        ...
        {/* Specs row */}
        {isPart ? (
          <div className={`flex flex-wrap items-center gap-x-3 gap-y-1 text-xs ${isPremium ? "text-slate-500" : "text-slate-500"}`}>
