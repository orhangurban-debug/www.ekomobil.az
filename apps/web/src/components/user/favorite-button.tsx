...
export function FavoriteButton({
  ...
  return (
    <button
      onClick={onToggle}
      disabled={loading}
      className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-900/10 bg-white/95 text-slate-500 shadow-sm transition hover:text-red-500"
      aria-label={favorited ? "Sevimlilərdən çıxar" : "Sevimlilərə əlavə et"}
    >
      <svg className={`h-5 w-5 ${favorited ? "fill-red-500 text-red-500" : ""}`} viewBox="0 0 20 20" fill="none" stroke="currentColor">
