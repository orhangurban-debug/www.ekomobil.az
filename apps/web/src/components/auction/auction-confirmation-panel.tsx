...
export function AuctionConfirmationPanel({
  ...
  return (
    <div className="rounded-2xl border border-slate-900/10 bg-white p-6 shadow-sm">
          ...
          {canActAsBuyer && (
            <>
              ...
              <button
                ...
                disabled={Boolean(loadingAction)}
                className="btn-secondary justify-center sm:col-span-2 border-amber-300 text-amber-700 hover:bg-amber-500/10"
              >
      ...
      {canRelist && (
        <div className="mt-4 rounded-xl border border-slate-900/10 bg-white/60 p-4">
      ...
      {relistResult && (
        <div className="mt-4 rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm text-emerald-700">
          <p className="font-semibold">Yeni lot yaradıldı: {relistResult.newAuctionId}</p>
          <p className="mt-1 text-emerald-700">Lot aktivləşməsi üçün ödəniş addımlarını tamamlayın.</p>
      ...
      {error && (
        <div className="mt-4 rounded-xl alert-danger border px-4 py-3 text-sm text-red-700">
          {error}
        </div>
      )}
    </div>
