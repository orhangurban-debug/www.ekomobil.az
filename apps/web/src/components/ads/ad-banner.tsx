  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 items-center gap-4">
        <div
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-xs font-bold text-slate-900"
      ...
      <div className="flex shrink-0 items-center gap-3">
        ...
        <Link
          href={content.href}
          className="rounded-lg px-4 py-2 text-xs font-semibold text-slate-900 transition hover:opacity-90"
          style={{ backgroundColor: content.accent }}
...
function RectangleAd({ content }: { content: NonNullable<AdBannerProps["demoContent"]> }) {
  return (
    <div
      className="flex h-full w-full flex-col items-center justify-between overflow-hidden rounded-xl border glass-panel border-slate-900/10 p-5 shadow-sm"
    >
      {/* Top: Reklam etiket */}
      <div className="flex w-full items-center justify-between">
        <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Reklam</span>
        <div
      ...
      <div className="flex flex-col items-center gap-3 text-center">
        <div
          className="flex h-14 w-14 items-center justify-center rounded-2xl text-lg font-bold text-slate-900 shadow-sm"
          style={{ backgroundColor: content.accent }}
        >
          {content.logoText.slice(0, 2).toUpperCase()}
        </div>
      ...
      <Link
        href={content.href}
        className="w-full rounded-xl py-2.5 text-center text-sm font-semibold text-slate-900 transition hover:opacity-90"
        style={{ backgroundColor: content.accent }}
        onClick={(e) => e.stopPropagation()}
      >
        {content.cta}
...
export function NativeAdCard({ slotLabel }: { slotLabel?: string }) {
  ...
  return (
    <div className="relative flex flex-col overflow-hidden rounded-2xl border glass-panel border-slate-900/10 shadow-sm">
      {/* "Reklam" işarəsi */}
      <div className="absolute right-3 top-3 z-10 rounded-md bg-white/63 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-widest text-slate-400">
        Reklam
      </div>
      ...
      >
        <div
          className="flex h-16 w-16 items-center justify-center rounded-2xl text-xl font-black text-slate-900 shadow"
          style={{ backgroundColor: content.accent }}
        >
          {content.logoText.slice(0, 2).toUpperCase()}
        </div>
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <Link
          href={content.href}
          className="mt-auto block w-full rounded-xl py-2.5 text-center text-sm font-semibold text-slate-900 transition hover:opacity-90"
          style={{ backgroundColor: content.accent }}
        >
          {content.cta}
        </Link>
