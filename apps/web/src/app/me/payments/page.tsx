...
const STATUS_BADGE = {
  ...
  pending: { label: "Hazırlanır", cls: "bg-amber-500/15 text-amber-700 border-amber-500/25" },
  failed: { label: "E-poçt göndərilmədi", cls: "bg-white/63 text-slate-600 border-slate-900/10" }
};
      ...
      {invoices.length === 0 ? (
        ...
        <div className="space-y-3">
          {invoices.map((inv) => {
            ...
            return (
              <div key={inv.id} className="rounded-2xl border glass-panel border-slate-900/10 p-5 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-semibold text-slate-900 text-sm">{inv.invoiceNumber}</span>
                      <span className="rounded-full border px-2 py-0.5 text-[11px] font-medium bg-white/63 text-slate-600 border-slate-900/10">
