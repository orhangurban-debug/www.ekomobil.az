...
export default async function ProfilePage({
  ...
  const statusMeta: Record<string, { label: string; cls: string }> = {
    active: { label: "Aktiv", cls: "bg-emerald-500/15 text-emerald-300 border-emerald-500/25" },
    pending_review: { label: "Yoxlamada", cls: "bg-amber-500/15 text-amber-200 border-amber-500/25" },
    draft: { label: "Qaralama", cls: "bg-white/8 text-white/65 border-white/10" },
    sold: { label: "Satılıb", cls: "bg-[#0057FF]/10 text-[#0057FF] border-brand-200" },
    rejected: { label: "Rədd edilib", cls: "bg-red-500/15 text-red-200 border-red-500/25" },
    archived: { label: "Arxiv", cls: "bg-white/8 text-white/50 border-white/10" },
    inactive: { label: "Deaktiv", cls: "bg-white/8 text-white/50 border-white/10" }
  };
    ...
    <div className="mx-auto max-w-5xl px-4 py-10">
      ...
      {welcome === "service" && (
        <div className="mb-6 rounded-2xl border border-emerald-500/25 bg-emerald-500/10 px-6 py-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-semibold text-emerald-300">Servis profili üçün müraciət</p>
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Mənim profilim</h1>
          <p className="mt-2 text-white/50">{profile?.email} • {profile?.city || "Şəhər qeyd olunmayıb"}</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        <div className="space-y-6">
          <section className="card p-6">
            <h2 className="font-semibold text-white">Profil məlumatları</h2>
            <dl className="mt-4 grid gap-4 sm:grid-cols-2">
              <div>
                <dt className="text-xs uppercase tracking-wider text-white/40">Ad</dt>
                <dd className="mt-1 text-sm font-medium text-white">{profile?.fullName || "Qeyd olunmayıb"}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-white/40">Hesab növü</dt>
                <dd className="mt-1 text-sm font-medium text-white">
                  {formatAccountTypeLabel(user.role, businessSnapshot)}
                </dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-white/40">Email statusu</dt>
                <dd className="mt-1 text-sm font-medium text-white">
                  {profile?.emailVerified ? "Təsdiqlənib" : "Təsdiqlənməyib"}
                </dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-white/40">Telefon</dt>
                <dd className="mt-1 text-sm font-medium text-white">{profile?.phone || "Qeyd olunmayıb"}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-white/40">Dərin identifikasiya</dt>
                <dd className="mt-1 text-sm font-medium text-white">
          ...
          <section className="card p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-semibold text-white">Auksion yenilikləri</h2>
              <Link href="/auction" className="btn-secondary text-sm">Auksiona keç</Link>
            </div>
            {auctionNotifications.length === 0 ? (
              <p className="text-sm text-white/50">Hələ auksion bildirişi yoxdur.</p>
            ) : (
              <div className="space-y-3">
                {auctionNotifications.map((item) => (
                  <div key={item.id} className={`rounded-xl border p-4 ${item.isRead ? "border-white/10" : "border-[#0057FF]/25 bg-[#0057FF]/5"}`}>
          ...
          <section className="card p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-semibold text-white">Mənim elanlarım</h2>
              <Link href="/publish" className="btn-secondary text-sm">Yeni elan</Link>
            </div>
            {hasNonActiveListings && (
              <div className="mb-4 rounded-xl alert-warning border px-3 py-2 text-xs text-amber-200">
            ...
            {myListings.length === 0 ? (
              <div className="rounded-xl border border-dashed border-white/10 py-8 text-center">
                <p className="text-sm text-white/50">Hələ elanınız yoxdur.</p>
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <Link href={`/listings/${item.id}`} className="font-medium text-white hover:text-[#0057FF]">
                            {item.title}
                          </Link>
                          <span className={`rounded-full border px-2 py-0.5 text-[11px] font-medium ${statusMeta[item.status]?.cls ?? "bg-white/8 text-white/65 border-white/10"}`}>
                            {statusMeta[item.status]?.label ?? item.status}
                          </span>
                        </div>
                        <div className="mt-1 text-xs text-white/50">{item.city} • {item.year}</div>
                      </div>
                      <div className="flex shrink-0 items-center gap-4">
                        ...
                        <div className="text-right">
                          ...
                          <div className="mt-1 text-xs text-white/50">Trust {item.trustScore}/100</div>
                        </div>
                      </div>
                    </div>
        ...
        <section className="space-y-6">
          ...
          <div className="card p-6">
            <h2 className="font-semibold text-white">Profil qısa statistikası</h2>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-white/5 p-4">
                <div className="text-2xl font-bold text-[#0057FF]">{favorites.length}</div>
                <div className="text-xs text-white/50">Favori elan</div>
              </div>
              <div className="rounded-2xl bg-white/5 p-4">
                <div className="text-2xl font-bold text-[#0057FF]">{savedSearches.length}</div>
                <div className="text-xs text-white/50">Yadda saxlanmış axtarış</div>
              </div>
              <div className="rounded-2xl bg-white/5 p-4 col-span-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-white/50">Son ödənişlər</div>
                  </div>
          ...
          {user.role === "viewer" && (
            <div className="card p-6">
              <h2 className="font-semibold text-white">Hesabı yüksəlt</h2>
              ...
              <div className="mt-4 rounded-xl border border-white/10 p-4">
                <p className="text-sm font-medium text-white/90">🔧 Servis / Usta profili</p>
                <p className="mt-1 text-xs text-white/50">Bu hesabla həm elan yerləşdirə, həm servis profili aça bilərsiniz</p>
          ...
          <div className="card p-6">
            <h2 className="font-semibold text-white">Yadda saxlanmış axtarışlar</h2>
            <div className="mt-4 space-y-3">
              {savedSearches.length === 0 ? (
                <p className="text-sm text-white/50">Hələ saxlanmış axtarış yoxdur.</p>
              ) : (
                savedSearches.map((search) => {
                  ...
                  return (
                    <div key={search.id} className="rounded-xl border border-white/10 p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="text-sm font-medium text-white">{search.name || "Axtarış"}</div>
                      ...
                      {parts.length > 0 && (
                        <div className="mt-1.5 flex flex-wrap gap-1">
                          {parts.map((p) => (
                            <span key={p} className="rounded-md bg-white/8 px-2 py-0.5 text-xs text-white/50">
                              {p}
                            </span>
                          ))}
                        </div>
