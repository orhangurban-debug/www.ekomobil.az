...
function SellerAvatar({ name }: { name: string }) {
  const initials = name
    ...
    .toUpperCase();
  return (
    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#0057FF] to-[#0046CC] text-xl font-bold text-white shadow">
      {initials || "EK"}
    </div>
  );
}
