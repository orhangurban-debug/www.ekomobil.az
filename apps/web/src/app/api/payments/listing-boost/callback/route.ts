...
import {
  finalizeListingBoostPayment,
  getListingBoostPayment,
  getListingBoostPaymentByRemoteOrderId
} from "@/server/listing-boost-payment-store";
import {
...
export async function GET(req: Request) {
  ...
  const payment = (await getListingBoostPayment(incomingId))
    ?? (await getListingBoostPaymentByRemoteOrderId(incomingId));
  if (!payment) {
  ...
  const result = await finalizeListingBoostPayment({
  ...
  const redirectUrl = new URL(`/payments/listing-boost/${payment.id}`, req.url);
  if (result.payment?.status === "succeeded") {
    redirectUrl.pathname = `/listings/${result.payment.listingId}`;
    redirectUrl.searchParams.set("boost", "success");
  }
