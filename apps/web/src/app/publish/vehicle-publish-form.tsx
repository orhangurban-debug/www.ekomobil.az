...
export function VehiclePublishForm({ dealerPublishContext }: { dealerPublishContext: DealerPublishContext }) {
  ...
  const vehicleStepErrors = useMemo(() => {
    ...
    return errors;
  }, [city, declaredMileageKm, engineVolumeCc, fuelType, make, model, ownersCount, priceAzn, title, vin, year]);
    ...
    () =>
      make.trim() && model.trim()
        ? buildVehicleListingTitle({
            ...
            year,
            fuelType,
            mileageKm: declaredMileageKm,
            color
          })
        : "",
    [color, declaredMileageKm, fuelType, make, model, year]
  );

  useEffect(() => {
    if (titleTouched || !autoGeneratedTitle) return;
    setTitle(autoGeneratedTitle);
  }, [autoGeneratedTitle, titleTouched]);
            ...
            {step === "Məlumatlar" && (
              <div className="card space-y-5 p-4 sm:p-8">
                ...
                <div>
                  ...
                  {!titleTouched && autoGeneratedTitle && (
                    <p className="mt-1 text-xs text-slate-500">
                      Başlıq marka, model, yürüş və rəngə görə avtomatik yenilənir.
                    </p>
                  )}
                </div>
